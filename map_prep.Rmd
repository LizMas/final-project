...
shape <- read_sf("gadm36_YEM_2.shp")

#do I need this .rds or should this x be the conflict and/or the migration data? 

x <- read_rds("yemen_map.rds") %>% 
  drop_na()

locations <- st_as_sf(x, coords = c("lng", "lat"), crs = 4326)

write_rds(locations, "clean-data/locations.rds")
write_rds(shape, "clean-data/shape.rds")
```
...
idp_map <- ggplot(shape) +
  geom_sf(data = shape) #+
  #geom_sf(data = conflict_dated, aes(color = sub_event_type))
