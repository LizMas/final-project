...
#why should I have to load this in again? 
...
#write rds for the maps 
 
write_rds(map_fat_actor1, "final_proj_shiny_app/map_fat_actor1.rds")

write_rds(map_fat_sub_event, "final_proj_shiny_app/map_fat_sub_event.rds")

```


```{r}
fat_actor1_model <- conflict_actor1_recode %>% 
  select(recode_actor1, event_date, fatalities) %>% 
  dplyr::group_by(recode_actor1) %>% 
  summarise(sum_fatby_actor1 = sum(fatalities)) %>% 
  filter(sum_fatby_actor1 != 0) %>% 
...
#this throws an error. Goal: make lm for fatalities and PGMs: 
model_pgm <-  lm(sum_fatby_actor1 ~ recode_actor1, data = fat_actor1_model)

model_pgm



#model_data <- conflict_actor1_recode$progov_militias <- ifelse(conflict_actor1_recode$recode_actor1 =="Pro-Government Militias", 1, 0) 

#model <- lm(fatalities ~ recode_actor1, data = model_data)

#conflict_actor1_recode %>% mutate(progov_militias = ifelse(recode_actor1 =="Pro-Government Militias", 1, 0))
```
